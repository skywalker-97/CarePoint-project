...
import { Users, Filter, ShieldCheck, CheckCircle, Clock, Loader2 } from 'lucide-react';
                    ...
                    <div key={index} className='group bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300'>
                        ...
                        <div className='p-6'>
                            <div className='mb-4'>
                                ...
                                <p className='text-xs text-gray-400 font-medium mt-1'>{item.degree || "N/A"} - {item.experience || "0"} Exp</p>
                            </div>
                                ...
                                <div className='flex items-center gap-4'>
                                    <div className='flex items-center gap-2'>
                                        {processing[item._id] ? (
                                            ...
                                            <input 
                                                onChange={() => handleAction(item._id, changeAvailability)} 
                                                type="checkbox" 
                                    ...
                                    <button 
                                        ...
                                        onClick={() => handleAction(item._id, changeVerification)}
                                        className={`p-2 rounded-lg transition-all ${item.isVerified ? 'bg-blue-50 text-blue-600 hover:bg-blue-100' : 'bg-amber-50 text-amber-600 hover:bg-amber-100'} ${processing[item._id] ? 'opacity-50 cursor-not-allowed' : ''}`}
                                        title={item.isVerified ? "Revoke Verification" : "Approve Physician"}
                                    >
                                        {processing[item._id] ? <Loader2 size={16} className="animate-spin" /> : <ShieldCheck size={16} />}
                                    </button>
                                </div>
