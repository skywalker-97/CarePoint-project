            ...
            <div className='flex flex-col sm:flex-row gap-4'>
                ...
                <div className='flex-1 border border-stone-100 rounded-lg p-8 py-7 bg-white'>
                    ...
                    <p className='flex items-center gap-2 text-3xl font-medium text-gray-700'>
                        {isEdit ? <input className='border border-stone-100 rounded-lg bg-stone-50 px-2' type="text" onChange={(e) => setProfileData(prev => ({ ...prev, name: e.target.value }))} value={profileData.name} /> : profileData.name}
                    </p>
                    ...
                    <div>
                        ...
                        <p className='text-sm text-gray-600 max-w-[700px] mt-1'>
                            {isEdit ? <textarea className='w-full border border-stone-100 rounded-lg bg-stone-50 p-2 mt-1' rows={4} onChange={(e) => setProfileData(prev => ({ ...prev, about: e.target.value }))} value={profileData.about}></textarea> : profileData.about}
                        </p>
                    </div>
                    ...
                    {
                        isEdit
                            ? <button onClick={updateUserProfile} className='px-4 py-1 border border-primary text-sm rounded-full mt-5 hover:bg-primary hover:text-white transition-all'>Save</button>
